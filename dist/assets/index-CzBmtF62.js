...
`).forEach(t=>{const o=document.createElement("p");o.innerHTML=t.replace(/\n/g,"<br>"),e.appendChild(o)}),j&&(j(),j=null)}function Q(){return z}function fe(e,a){const n=document.getElementById("choices-container");n.innerHTML="",n.classList.remove("visible"),setTimeout(()=>{e.forEach((t,o)=>{const s=document.createElement("button");s.className="choice-btn",s.textContent=t.text,s.style.animationDelay=`${o*.15}s`,s.addEventListener("click",()=>{n.querySelectorAll(".choice-btn").forEach(i=>{i.disabled=!0,i.classList.add("disabled")}),s.classList.add("chosen"),a(o)}),n.appendChild(s)}),n.classList.add("visible")},300)}function ge(){const e=document.getElementById("choices-container");e.innerHTML="",e.classList.remove("visible")}function v(e){document.querySelectorAll(".screen").forEach(n=>{n.classList.remove("active")});const a=document.getElementById(e);a&&a.classList.add("active")}function Y(e="white",a=150){const n=document.getElementById("flash-overlay");n.style.backgroundColor=e,n.classList.add("active"),setTimeout(()=>n.classList.remove("active"),a)}function R(e=400){const a=document.getElementById("glitch-layer");a.classList.add("active"),setTimeout(()=>a.classList.remove("active"),e)}function he(e){const a=document.documentElement,n=Math.min(e.miedo/100,1);a.style.setProperty("--vignette-intensity",.3+n*.5);const t=Math.max(e.cordura/100,0);a.style.setProperty("--saturation",t),a.style.setProperty("--noise-opacity",.03+(1-t)*.08);const o=Math.min(e.paranoia/100,1);a.style.setProperty("--distortion",o*2),document.body.classList.toggle("high-fear",e.miedo>60),document.body.classList.toggle("low-sanity",e.cordura<40),document.body.classList.toggle("high-paranoia",e.paranoia>50)}function ve(e){const a=document.getElementById("game-screen");a.classList.add("scene-transition"),setTimeout(()=>{e(),setTimeout(()=>{a.classList.remove("scene-transition")},600)},400)}function be(e,a,n){const t=document.getElementById("endings-list");t.innerHTML="",Object.keys(a).forEach(i=>{const c=a[i],u=e.find(q=>q.id===i),l=document.createElement("div");l.className="ending-card"+(u?" unlocked":" locked"),u?l.innerHTML=`
      ...
      `,t.appendChild(l)});const s=document.createElement("p");s.className="endings-counter",s.textContent=`${e.length} / ${n} finales desbloqueados`,t.appendChild(s)}function xe(e,a={}){const n=document.getElementById("ending-title"),t=document.getElementById("ending-type-label"),o=document.getElementById("ending-text");n.textContent=e.endingTitle;const s={malo:"FINAL MALO",neutro:"FINAL NEUTRO",perturbador:"FINAL PERTURBADOR",ambiguo:"FINAL AMBIGUO"};t.textContent=s[e.endingType]||"FINAL",t.className=`ending-type-badge ending-type-${e.endingType}`;const i=typeof e.getText=="function"?e.getText(a):e.getText;return o.innerHTML="",X(o,i,40)}let T=!1,G=null,m=null;const ye=.85,Ee=.75;function Te(){if(!("speechSynthesis"in window))return;const e=()=>{const a=speechSynthesis.getVoices();a.length!==0&&(G=a.find(n=>n.lang.startsWith("es")&&n.localService)||a.find(n=>n.lang.startsWith("es"))||a.find(n=>n.lang.startsWith("en")&&n.localService)||a[0])};e(),speechSynthesis.onvoiceschanged=e}function Se(){return T=!T,T||A(),T}function qe(){return"speechSynthesis"in window}function ee(e){if(!T||!("speechSynthesis"in window))return;A();const a=e.replace(/\n+/g,". ").replace(/["""''«»]/g,"").replace(/—/g,", ").replace(/\.\.\./g,"...").replace(/\s{2,}/g," ").trim();a&&(m=new SpeechSynthesisUtterance(a),m.rate=ye,m.pitch=Ee,m.volume=.9,G&&(m.voice=G),m.lang="es-ES",m.onend=()=>{m=null},speechSynthesis.speak(m))}function A(){"speechSynthesis"in window&&speechSynthesis.cancel(),m=null}let r={currentScene:"inicio",variables:{...H},history:[]},$="";function J(){Le(),je(),_e(),v("menu-screen"),re()&&(document.getElementById("btn-continue").style.display="",document.getElementById("btn-delete").style.display=""),Te()}function Le(){document.getElementById("btn-new-game").addEventListener("click",()=>{P(),ae()}),document.getElementById("btn-continue").addEventListener("click",()=>{P(),Ce()}),document.getElementById("btn-endings").addEventListener("click",()=>{P(),Oe()}),document.getElementById("btn-delete").addEventListener("click",()=>{confirm("¿Borrar todo el progreso? Esta acción no se puede deshacer.")&&(N(),document.getElementById("btn-continue").style.display="none",document.getElementById("btn-delete").style.display="none",S())}),document.getElementById("btn-restart").addEventListener("click",()=>{A(),N(),v("menu-screen"),document.getElementById("btn-continue").style.display="none",document.getElementById("btn-delete").style.display="none",S()}),document.getElementById("btn-back-menu").addEventListener("click",()=>{v("menu-screen"),S()})}function _e(){const e=document.getElementById("btn-narration");if(!qe()){e.style.display="none";return}e.addEventListener("click",()=>{const a=Se(),n=e.querySelector(".narration-status");n.textContent=a?"ON":"OFF",e.classList.toggle("active",a),e.title=a?"Narración activada":"Narración desactivada",S()})}function je(){document.getElementById("narrative-container").addEventListener("click",e=>{if(Q()){e.stopPropagation();const a=document.getElementById("narrative-text");F(a,$)}}),document.addEventListener("keydown",e=>{if((e.code==="Space"||e.code==="Enter")&&Q()){e.preventDefault();const a=document.getElementById("narrative-text");F(a,$)}})}function ae(){r={currentScene:"inicio",variables:{...H},history:[]},N(),U(),Y("black",800),setTimeout(()=>{v("game-screen"),O("inicio")},600)}function Ce(){const e=ce();if(!e){ae();return}r.currentScene=e.currentScene,r.variables={...H,...e.variables},r.history=e.history||[],U(),v("game-screen"),O(r.currentScene)}function O(e){const a=ne[e];if(!a){console.error(`Scene not found: ${e}`);return}r.currentScene=e,C(),ge(),he(r.variables);const n=Math.min((r.variables.miedo+r.variables.paranoia)/150,1);de(n);const t=typeof a.getText=="function"?a.getText(r.variables):a.getText;if($=t,a.isEnding){Ae(a);return}ve(()=>{const o=document.getElementById("narrative-text");X(o,t,28).then(()=>{Ne(a)}),ee(t),a.timeLimit&&Ie(a),ie(r)})}function Ne(e){const n=e.choices.filter(t=>t.condition?t.condition(r.variables):!0).map(t=>({...t,text:typeof t.text=="function"?t.text(r.variables):t.text}));fe(n,t=>{ze(n[t])})}function ze(e){S(),C(),A(),e.effects&&(Object.entries(e.effects).forEach(([n,t])=>{r.variables[n]!==void 0?r.variables[n]+=t:r.variables[n]=t}),r.variables.miedo=Math.max(0,Math.min(100,r.variables.miedo)),r.variables.cordura=Math.max(0,Math.min(100,r.variables.cordura)),r.variables.paranoia=Math.max(0,Math.min(100,r.variables.paranoia)),r.variables.culpa=Math.max(0,Math.min(100,r.variables.culpa)),r.variables.trauma=Math.max(0,Math.min(100,r.variables.trauma))),r.history.push({scene:r.currentScene,choice:e.text});const a=typeof e.nextScene=="function"?e.nextScene(r.variables):e.nextScene;setTimeout(()=>{R(200),U(),setTimeout(()=>O(a),300)},200)}function Ie(e){const a={done:!1};let n=0;se(e.timeLimit,()=>{if(Y("rgba(139, 0, 0, 0.4)",300),R(500),V(),e.timeoutScene){const t=e.choices.find(o=>o.isTimeoutDefault);t&&(r.history.push({scene:r.currentScene,choice:"[tiempo agotado]"}),t.effects&&Object.entries(t.effects).forEach(([o,s])=>{r.variables[o]!==void 0?r.variables[o]+=s:r.variables[o]=s})),setTimeout(()=>O(e.timeoutScene),400)}},(t,o)=>{const s=t/o,i=Date.now();s<.4&&!a.done&&(a.done=!0,V()),s<.3&&i-n>800&&(n=i,me()),s<.3&&Math.random()<.05&&(document.getElementById("game-screen").classList.add("flicker"),setTimeout(()=>{document.getElementById("game-screen").classList.remove("flicker")},100)),s<.15&&Math.random()<.08&&R(100)})}function Ae(e){le(e.id,e.endingTitle,e.endingType),N(),pe(),Y("black",1500),setTimeout(()=>{v("ending-screen"),xe(e,r.variables);const a=typeof e.getText=="function"?e.getText(r.variables):e.getText;ee(a)},1200)}function Oe(){const e=W();be(e,oe,te),v("endings-screen")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",J):J();
